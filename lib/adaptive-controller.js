...
class AdaptiveController {
    ...
    update(currentTemp) {
        ...
        // Check if within hysteresis zone - stable, set to target temperature
        if (Math.abs(error) < this.hysteresis && this.lastOutput !== null) {
            // When stable, set exactly to target (rounded to precision)
            const stableOutput = this.roundToPrecision(this.targetTemp);
            this.lastOutput = stableOutput;
            return this.createOutput(stableOutput, currentTemp, error, 'stable', null, activeMode);
        }

        // Check if action is needed based on mode
        if (this.mode === 'heat' && error < 0) {
            // In heat mode, don't act if room is too warm (just maintain target)
        ...
        if (this.mode === 'cool' && error > 0) {
            // In cool mode, don't act if room is too cold (just maintain target)
            const output = this.roundToPrecision(this.targetTemp);
            this.lastOutput = output;
            return this.createOutput(output, currentTemp, error, 'idle', null, activeMode);
        }
        ...
        // Heat: setpoint > target to heat up (at least one precision step above target)
        // Cool: setpoint < target to cool down (at least one precision step below target)
        let rawOutput;
        if (activeMode === 'cool') {
            // For cooling, setpoint should be below target
            rawOutput = this.targetTemp - pidResult.adjustment;
            // Ensure at least one precision step below target when actively cooling
            ...
            rawOutput = this.targetTemp + pidResult.adjustment;
            // Ensure at least one precision step above target when actively heating
    ...
    calculatePID(error, dt, activeMode) {
        // For cooling mode, we use absolute error (always positive for control)
        const absError = Math.abs(error);

        // Proportional term
        const P = this.Kp * absError;

        // Integral term with anti-windup (use absolute error for integral)
        this.integral += absError * dt;
    ...
    getScheduledTemp() {
        ...
        const daySchedule = this.schedule[dayNames[now.getDay()]];

        if (!daySchedule || !Array.isArray(daySchedule) || daySchedule.length === 0) {
            return this.schedule.default !== undefined ? this.schedule.default : this.baseTargetTemp;
        }
        ...
        // If no slot before current time, use last slot from previous day
        if (!activeSlot) {
            const yesterday = dayNames[(now.getDay() + 6) % 7];
            const yesterdaySchedule = this.schedule[yesterday];
    ...
    calculateEffectiveTarget() {
        ...
        // In schedule mode, get temp from schedule
    ...
    getCurrentScheduleSlot() {
        if (!this.schedule || this.operatingMode !== 'schedule') return null;
